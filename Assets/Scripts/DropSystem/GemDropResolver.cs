using System.Collections.Generic;
using Battle;
using InventorySystem;

namespace DropSystem
{
    public class GemDropResolver
    {
        public List<InventoryItem> Resolve(GemDropTable dropTable, GemDropContext context = null)
        {
            List<InventoryItem> droppedItems = new();
            if (dropTable == null)
                return droppedItems;

            IReadOnlyList<GemDropEntry> entries = dropTable.Entries;
            for (int i = 0; i < entries.Count; i++)
            {
                GemDropEntry entry = entries[i];
                if (entry == null)
                    continue;

                if (!entry.ShouldDrop(context))
                    continue;

                InventoryItem item = entry.CreateDroppedItem();
                if (item != null)
                    droppedItems.Add(item);
            }

            return droppedItems;
        }

        public List<InventoryItem> Resolve(EnemySpawnData spawnData)
        {
            if (spawnData?.Archetype == null)
                return new List<InventoryItem>();

            return Resolve(spawnData.Archetype.GemDropTable, GemDropContext.FromSpawnData(spawnData));
        }

        public List<InventoryItem> Resolve(EnemyUnit enemyUnit)
        {
            if (enemyUnit == null)
                return new List<InventoryItem>();

            return Resolve(enemyUnit.SpawnData);
        }
    }
}
